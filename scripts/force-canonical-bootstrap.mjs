...
express.application.post = function patchedPost(
  path,
  ...handlers
) {
  if (path !== "/api/stories")
  ...
  const saveStory = async (req, res) => {
    ...
    if (!publicId || !secureUrl) {
      return res
        ...
        .json({
          ok: false,
          error:
            "Uploaded story data is required.",
        });
    }
    ...
    try {
      ...
      await db.ref().update({
        ...
        [`users/${user.uid}/content/stories/${ref.key}`]:
          story,
      });
      await db
        .ref(
          `users/${user.uid}/stats/storiesCount`,
        )
        .transaction(
          (current) => (Number(current) || 0) + 1,
        );
      console.log(
        `[story-save] saved ${ref.key} for ${user.uid}`,
      );
      return res
        .status(201)
        .json({ ok: true, story });
