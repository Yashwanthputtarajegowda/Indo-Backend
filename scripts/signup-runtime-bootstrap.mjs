...
const validUserId = (value) => /^[a-z0-9._-]{1,50}$/.test(value);
const userIdKey = (value) => String(value || "").replace(/\./g, "%2E").replace(/#/g, "%23").replace(/\$/g, "%24").replace(/\//g, "%2F").replace(/\[/g, "%5B").replace(/\]/g, "%5D");
...
async function findTakenUserId(userId, exceptUid = "") {
  ...
  for (const [uid, value] of Object.entries(users)) {
    if (String(uid) === String(exceptUid || "")) continue;
    const existing = normalizeUserId(value?.profile?.userId || value?.profile?.username || value?.userId || value?.username || "");
  ...
  express.application.post = function signupRuntimePost(path, ...handlers) {
    if (path === "/api/account/check-user-id") {
      const handler = async (req, res) => {
        ...
        try {
          const taken = await findTakenUserId(userId);
    ...
    if (path === "/api/account/claim-user-id") {
      const handler = async (req, res) => {
        ...
        try {
          if (await findTakenUserId(userId, uid)) return res.status(409).json({ ok: false, error: `@${userId} is already taken.` });
          ...
          return res.status(500).json({ ok: false, error: "Could not create the Indo profile.", detail: String(error?.message || error || "Unknown error") });
        }
      };
      return originalAppPost.call(this, path, handler);
    }

    return originalAppPost.call(this, path, ...handlers);
  };
}

await import("./signup-canonical-bootstrap.mjs");
