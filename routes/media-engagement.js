...
export function createMediaEngagementRouter({ db, requireUser }) {
  ...
  router.get('/earnings/summary', async (req, res) => {
    ...
    try {
      ...
      await db.ref(`users/${user.uid}/earning/summary`).update({ videoViews, reelViews, grossRevenue, payableRevenue, updatedAt: Date.now() });
      return res.json({ ok: true, videoViews, reelViews, grossRevenue, payableRevenue, earningEnabled: Boolean(earning.enabled), rates: { videoPer1000Views: VIDEO_RATE_PER_1000_VIEWS, reelPer1000Views: REEL_RATE_PER_1000_VIEWS } });
  ...
  router.get('/wallet', async (req, res) => {
    ...
    try {
      ...
      return res.json({ ok: true, balance: Number(summary.payableRevenue || 0), earningEnabled: Boolean(earning.enabled), minimumPayoutUsd: MIN_PAYOUT_USD, payouts });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || 'Could not load wallet.' }); }
  });

  router.post('/wallet/payout-request', async (req, res) => {
    const user = await requireUser(req, res); if (!user) return;
    if (!db) return res.status(503).json({ ok: false, error: 'Firebase Admin is not configured on the backend.' });
    ...
    try {
      ...
      if (balance < MIN_PAYOUT_USD) return res.status(400).json({ ok: false, error: `Minimum payout is $${MIN_PAYOUT_USD.toFixed(2)}.` });
      if (requestedAmount > balance) return res.status(400).json({ ok: false, error: 'Requested amount is greater than your available balance.' });
      const payoutRef = db.ref(`users/${user.uid}/payouts`).push();
      const payout = { id: payoutRef.key, amount: Number(requestedAmount.toFixed(2)), method, status: 'pending', createdAt: Date.now() };
      await payoutRef.set(payout);
      return res.status(201).json({ ok: true, payout });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || 'Could not create payout request.' }); }
  });
