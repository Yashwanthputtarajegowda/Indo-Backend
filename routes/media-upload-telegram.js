...
router.post(
  ...
  async (req, res) => {
    const user = await req.app.locals.requireUser?.(req, res);
    if (!user) return;
...
export function createTelegramMediaUploadRouter({ db, requireUser }) {
  // Expose dependencies through app.locals so this standalone router has no hidden imports.
  router.use((req, _res, next) => {
    req.app.locals.db = db;
    req.app.locals.requireUser = requireUser;
    next();
  });
  return router;
}
