...
function mixPriority(items, requesterUid, followingUids) {
  const followingSet = followingUids instanceof Set ? followingUids : new Set();
  const scored = items.map((item, index) => {
    const ownerUid = String(item?.ownerUid || "");
    const createdAt = Number(item?.createdAt || 0);
    const isFollowed = followingSet.has(ownerUid) || ownerUid === requesterUid;
    const hoursOld = Math.max(0, (Date.now() - createdAt) / 36e5);
    const freshness = Math.max(0, 1000 - Math.min(1000, hoursOld * 8));
    const followedBoost = isFollowed ? 100000 : 0;
    const freshBoost = freshness * 100;
    const jitter = Math.random() * 25;
    return { item, score: followedBoost + freshBoost + jitter, index };
  });
