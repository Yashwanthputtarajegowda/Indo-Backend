...
export async function isBlockedEitherWay({
  ...
  if (
    ...
    return false;
  const [requesterBlocked, ownerBlocked] =
    await Promise.all([
      db
        .ref(
...
export async function canViewPrivateMedia({
  ...
  if (!header.startsWith("Bearer ")) {
    res.status(401).json({
      ok: false,
      error:
        "Login required to view this private account.",
    });
    return false;
  }
  ...
  const follower = await db
    .ref(
      `users/${ownerUid}/followers/${requester.uid}`,
    )
    .get();
  if (!follower.exists()) {
    res.status(403).json({
      ok: false,
      error:
        "Follow this private account to view its content.",
    });
    return false;
  }
  return true;
...
function mixPriority(
  ...
  const scored = items.map((item, index) => {
    ...
    const isFollowed =
      followingSet.has(ownerUid) ||
      ownerUid === requesterUid;
    const hoursOld = Math.max(
      0,
      (Date.now() - createdAt) / 36e5,
    );
    const freshness = Math.max(
      0,
      1000 - Math.min(1000, hoursOld * 8),
    );
    const followedBoost = isFollowed ? 100000 : 0;
    const freshBoost = freshness * 100;
    const jitter = Math.random() * 25;
    return {
      item,
      score: followedBoost + freshBoost + jitter,
      index,
    };
  });
  ...
  // Keep followed content first, while shuffling it slightly so refreshes do not
  // always produce the exact same order. Then show the freshest general content,
  // followed by the rest of the normal uploads.
  const shuffle = (array) => {
    const copy = [...array];
...
export function createSocialBlockRouter({
  db,
  requireUser,
}) {
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        for (const story of stories) {
          ...
          const profile =
            (
              await profileCache.get(ownerUid)
            ).val() || {};
          if (
  ...
  router.get(
    ...
    async (req, res, next) => {
      ...
      try {
        ...
        // Feed order on every refresh:
        // 1) content from people the viewer follows (including newest first within that group),
        // 2) freshest general uploads, with a small shuffle so refreshes feel mixed,
        // 3) older normal uploads.
        const ordered = mixPriority(
          visible,
          requester?.uid || "",
          followingUids,
        );
