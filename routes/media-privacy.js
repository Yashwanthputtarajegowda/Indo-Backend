...
async function canAccessMedia({ db, user, media }) {
  const ownerUid = String(media.ownerUid || '');
  if (!ownerUid) return false;
  if (await blockedEitherWay(db, user.uid, ownerUid)) return false;
...
export function createMediaPrivacyRouter({ db, requireUser }) {
  ...
  async function loadAllowedMedia(req, res) {
    ...
    if (!mediaId) {
      res.status(400).json({ ok: false, error: 'Media ID is required.' });
      return null;
    }
  ...
  router.post('/media/:mediaId/like', async (req, res, next) => {
    const route = await loadAllowedMedia(req, res).catch(() => null);
    if (!route) return;
    ...
    try {
      ...
      if (like === wasLiked) return res.json({ ok: true, liked: like, likes: Number(media.likes || 0) });
  ...
  router.get('/media/:mediaId/engagement', async (req, res, next) => {
    const route = await loadAllowedMedia(req, res).catch(() => null);
    if (!route) return;
    const { user, mediaId, media } = route;
    try {
  ...
  router.post('/media/:mediaId/save', async (req, res, next) => {
    const route = await loadAllowedMedia(req, res).catch(() => null);
    if (!route) return;
  ...
  router.post('/media/:mediaId/comments', async (req, res, next) => {
    const route = await loadAllowedMedia(req, res).catch(() => null);
    if (!route) return;
    const { user, mediaId, media } = route;
    ...
    try {
      ...
      if (media.ownerUid !== user.uid) {
        await createNotification({ db, recipientUid: media.ownerUid, type: 'comment', actorUid: user.uid, actorName: profile.name || 'Indo User', actorUserId: comment.username, text: 'commented on your video', targetId: mediaId });
      }
