...
async function calculateSummary({ db, uid }) {
  const [userSnapshot, videosSnapshot] =
    await Promise.all([
  ...
  const videoViews = videos
    ...
    .reduce(
      (sum, item) => sum + asNumber(item.views),
      0,
    );
  ...
  const videoWatchSeconds = asNumber(
    user.earnings?.videoWatchSeconds,
  );
  const reelWatchSeconds = asNumber(
    user.earnings?.reelWatchSeconds,
  );
  const videoWatchHours = hours(
    videoWatchSeconds,
  );
  ...
  const earningOn =
    user.earnings?.enabled === true;
  const videoRevenue = earningOn
    ? (videoViews / 1000) * 0.5
    : 0;
  const reelRevenue = earningOn
    ? (reelViews / 1000) * 0.1
    : 0;
  const estimatedRevenue =
    videoRevenue + reelRevenue;
  ...
  return {
    ...
    earningOn,
    rates: {
      videoPerThousandViews: 0.5,
      reelPerThousandViews: 0.1,
    },
    videoRevenue,
    reelRevenue,
    estimatedRevenue,
    payableBalance: earningOn
      ? estimatedRevenue
      : 0,
  };
}
      ...
      try {
        ...
        if (enabled && !summary.eligible) {
          return res.status(403).json({
            ...
            eligible: false,
            videoWatchHours:
              summary.videoWatchHours,
            reelWatchHours:
              summary.reelWatchHours,
          });
        }
        await db
    ...
    async (req, res) => {
      ...
      if (!mediaId || requestedSeconds <= 0)
        return res.status(400).json({
          ok: false,
          error:
            "Media ID and positive watch time are required.",
        });
