...
async function calculateSummary({ db, uid }) {
  ...
  const videoRevenue = earningOn ? (videoViews / 1000) * 0.5 : 0;
  const reelRevenue = earningOn ? (reelViews / 1000) * 0.1 : 0;
  ...
  return {
    ...
    eligible,
    earningOn,
    rates: { videoPerThousandViews: 0.5, reelPerThousandViews: 0.1 },
    videoRevenue,
    reelRevenue,
    estimatedRevenue,
