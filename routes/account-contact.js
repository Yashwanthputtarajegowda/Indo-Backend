...
export function createAccountContactRouter({ db, auth, requireUser }) {
  ...
  router.get('/messages/:targetUid', async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ ok: false, error: error.message || 'Could not load messages.' });
    }
  });
  ...
  router.post('/messages/:targetUid/read', async (req, res) => {
    ...
    try {
      ...
      for (const [id, message] of Object.entries(messages)) {
        if (message?.recipientUid === user.uid && !message.readByRecipient) updates[`messages/${key}/${id}/readByRecipient`] = true;
      }
      if (Object.keys(updates).length) await db.ref().update(updates);
      await db.ref(`messageInbox/${user.uid}/${key}/unread`).set(false);
      return res.json({ ok: true });
    } catch (error) {
      return res.status(500).json({ ok: false, error: error.message || 'Could not mark messages read.' });
    }
  });
  ...
  router.get('/messages', async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({ ok: false, error: error.message || 'Could not load conversations.' });
    }
  });
