...
async function uploadProfileAvatar({ uid, data }) {
  ...
  const signature = createCloudinarySignature(timestamp, { folder, public_id: publicId });
  const body = new URLSearchParams({ file: avatarData, api_key: apiKey, timestamp: String(timestamp), folder, public_id: publicId, signature, overwrite: "true", invalidate: "true" });
...
export function createAccountContactRouter({ db, auth, requireUser }) {
  ...
  router.patch("/account/profile", async (req, res) => {
    ...
    const name = clean(req.body?.name, 80); const bio = clean(req.body?.bio, 160); const location = clean(req.body?.location, 100);
    const website = clean(req.body?.website, 200); const role = clean(req.body?.role, 60); const interests = clean(req.body?.interests, 240); const language = clean(req.body?.language, 40);
    const visibility = req.body?.visibility === "private" ? "private" : "public";
    if (!name) return res.status(400).json({ ok: false, error: "User Name is required." });
    try {
      const userRef = db.ref(`users/${user.uid}`); const snapshot = await userRef.get();
  ...
  router.patch("/account/contact", async (req, res) => {
    const user = await requireUser(req, res); if (!user) return;
    if (!db) return res.status(503).json({ ok: false, error: "Firebase Admin is not configured on the backend." });
    const mobile = String(req.body?.mobile || "").trim(); const email = String(req.body?.email || user.email || "").trim().toLowerCase();
    ...
    try {
      const userRef = db.ref(`users/${user.uid}`); const snapshot = await userRef.get();
      if (!snapshot.exists()) return res.status(404).json({ ok: false, error: "Profile not found." });
      await userRef.update({ mobile, email, contactUpdatedAt: Date.now() }); return res.json({ ok: true, mobile, email });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || "Could not save contact details." }); }
  });

  router.get("/account/public-profile/:uid", async (req, res) => {
    const viewer = await requireUser(req, res); if (!viewer) return;
    if (!db) return res.status(503).json({ ok: false, error: "Firebase Admin is not configured on the backend." });
    const uid = String(req.params.uid || "").trim(); if (!uid) return res.status(400).json({ ok: false, error: "User ID is required." });
    try {
      const snapshot = await db.ref(`users/${uid}`).get(); if (!snapshot.exists()) return res.status(404).json({ ok: false, error: "Profile not found." });
  ...
  router.get("/messages/:targetUid", async (req, res) => {
    const user = await requireUser(req, res); if (!user) return;
    if (!db) return res.status(503).json({ ok: false, error: "Firebase Admin is not configured on the backend." });
    const targetUid = clean(req.params.targetUid, 160); if (!targetUid || targetUid === user.uid) return res.status(400).json({ ok: false, error: "Valid recipient is required." });
    try {
      const targetSnapshot = await db.ref(`users/${targetUid}`).get(); if (!targetSnapshot.exists()) return res.status(404).json({ ok: false, error: "Recipient profile not found." });
      if (await isBlockedEitherWay(db, user.uid, targetUid)) return res.status(403).json({ ok: false, error: "Messaging is unavailable because one account has blocked the other." });
      const key = [user.uid, targetUid].sort().join("_"); const snapshot = await db.ref(`messages/${key}`).limitToLast(100).get();
      const messages = Object.values(snapshot.val() || {}).sort((a, b) => Number(a.createdAt || 0) - Number(b.createdAt || 0));
      return res.json({ ok: true, messages, recipient: publicProfile(targetSnapshot.val() || {}) });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || "Could not load messages." }); }
  });

  router.post("/messages/:targetUid", async (req, res) => {
    const user = await requireUser(req, res); if (!user) return;
    if (!db) return res.status(503).json({ ok: false, error: "Firebase Admin is not configured on the backend." });
    const targetUid = clean(req.params.targetUid, 160); const text = clean(req.body?.text, 1000);
    ...
    try {
      ...
      if (await isBlockedEitherWay(db, user.uid, targetUid)) return res.status(403).json({ ok: false, error: "Messaging is unavailable because one account has blocked the other." });
      const sender = senderSnapshot.val() || {}; const key = [user.uid, targetUid].sort().join("_"); const ref = db.ref(`messages/${key}`).push();
      const message = { id: ref.key, senderUid: user.uid, recipientUid: targetUid, senderUserId: sender.username || `@${user.uid.slice(0, 8)}`, text, readByRecipient: false, createdAt: Date.now() };
      await ref.set(message);
      const senderAvatar = String(sender.profile?.avatarUrl || sender.profile?.photoURL || sender.avatarUrl || sender.photoURL || ""); const recipient = targetSnapshot.val() || {};
      ...
      await db.ref(`messageInbox/${targetUid}/${key}`).set({ conversationId: key, otherUid: user.uid, otherUserId: sender.username || `@${user.uid.slice(0, 8)}`, otherName: sender.name || "Indo User", otherAvatarUrl: senderAvatar, lastMessage: text, updatedAt: message.createdAt, unread: true });
      await db.ref(`messageInbox/${user.uid}/${key}`).set({ conversationId: key, otherUid: targetUid, otherUserId: recipient.username || `@${targetUid.slice(0, 8)}`, otherName: recipient.name || "Indo User", otherAvatarUrl: recipientAvatar, lastMessage: text, updatedAt: message.createdAt, unread: false });
      return res.status(201).json({ ok: true, message });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || "Could not send message." }); }
  });
  ...
  router.post("/messages/:targetUid/read", async (req, res) => {
    ...
    const targetUid = clean(req.params.targetUid, 160); if (!targetUid || targetUid === user.uid) return res.status(400).json({ ok: false, error: "Valid recipient is required." });
    try {
      const key = [user.uid, targetUid].sort().join("_"); const snapshot = await db.ref(`messages/${key}`).limitToLast(100).get(); const messages = snapshot.val() || {}; const updates = {};
      ...
      if (Object.keys(updates).length) await db.ref().update(updates); await db.ref(`messageInbox/${user.uid}/${key}/unread`).set(false); return res.json({ ok: true });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || "Could not mark messages read." }); }
  });
  ...
  router.get("/messages", async (req, res) => {
    ...
    if (!db) return res.status(503).json({ ok: false, error: "Firebase Admin is not configured on the backend." });
    try {
      const snapshot = await db.ref(`messageInbox/${user.uid}`).get(); const conversations = Object.values(snapshot.val() || {}).sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
      return res.json({ ok: true, conversations });
    } catch (error) { return res.status(500).json({ ok: false, error: error.message || "Could not load conversations." }); }
  });
