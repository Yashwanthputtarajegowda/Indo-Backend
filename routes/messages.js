...
export function createMessagesRouter({
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        const context =
          await requireMessagingUsers(req, res);
        if (!context) return;
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        await db
          .ref(
            `conversations/${context.user.uid}/${context.targetUid}`,
          )
          .update({
            uid: context.targetUid,
            username:
              context.target.username ||
              `@${context.targetUid.slice(0, 8)}`,
            name:
              context.target.name || "Indo User",
            lastMessage: text,
        ...
        await db
          .ref(
            `conversations/${context.targetUid}/${context.user.uid}`,
          )
          .update({
  ...
  router.get("/messages", async (req, res) => {
    ...
    try {
      const snapshot = await db
        .ref(`conversations/${user.uid}`)
        .get();
      const conversations = Object.values(
        snapshot.val() || {},
      ).sort(
        (a, b) =>
          Number(b.lastMessageAt || 0) -
          Number(a.lastMessageAt || 0),
      );
      return res.json({
        ok: true,
        conversations,
