...
async function legacyUserIdTaken(
  db,
  userId,
  uid,
) {
...
async function findUidByUserId(db, identifier) {
  ...
  const direct = await db
    .ref(`usernames/${userIdKey(cleanId)}`)
    .get();
  if (direct.exists() && direct.val()?.uid)
    return String(direct.val().uid);
  const users =
...
export function createAccountClaimRouter({
  db,
  requireUser,
}) {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        if (
          await legacyUserIdTaken(db, userId, uid)
        ) {
          return res
            .status(409)
            .json({
        ...
        if (
          ...
          return res.status(500).json({
            ok: false,
            error:
              "Could not verify the Indo profile write.",
          });
        }
        ...
        console.error(
          "Canonical account claim failed:",
          error,
        );
        return res.status(500).json({
          ok: false,
          error:
            "Could not create the Indo profile.",
          detail: String(
            error?.message ||
              error ||
              "Unknown error",
    ...
    async (req, res) => {
      ...
      try {
        const indexed = await db
          .ref(`usernames/${userIdKey(userId)}`)
          .get();
        if (indexed.exists())
          return res.json({
            ok: true,
        ...
        const taken = await legacyUserIdTaken(
          db,
          userId,
          "",
        );
        ...
        console.error(
          "User ID availability check failed:",
          error,
        );
        return res.status(500).json({
          ok: false,
          error:
            "Could not check User ID. Please try again.",
        });
      }
    },
  );
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      try {
        const uid = await findUidByUserId(
          db,
  ...
  router.post(
    "/account/profile/avatar-signature",
    async (req, res) => {
      const user = await requireUser(req, res);
      if (!user) return;
      try {
        ...
        console.error(
          "Profile avatar signature failed:",
          error,
        );
