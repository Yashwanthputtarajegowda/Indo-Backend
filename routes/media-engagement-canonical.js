...
export function createCanonicalMediaEngagementRouter({
  db,
  requireUser,
}) {
  ...
  async function handleUniqueView(
    req,
    res,
    mediaId,
  ) {
  ...
  router.post(
    ...
    (req, res) =>
      handleUniqueView(
        req,
        res,
        safeText(req.params.mediaId, 120),
      ),
  );
  router.post(
    ...
    (req, res) =>
      handleUniqueView(
        req,
        res,
        safeText(req.params.videoId, 120),
      ),
  );
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const likeUsers =
          likeState.snapshot.val() || {};
        const likes = countTruthy(likeUsers);
        await db
          .ref(`videos/${mediaId}/likes`)
          .set(likes);
        if (media.ownerUid)
