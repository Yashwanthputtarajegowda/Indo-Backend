...
app.post("/api/media/videos", async (req, res) => {
  ...
  if (!publicId || !secureUrl)
    return res
      .status(400)
      .json({ ok: false, error: "Uploaded video data is required." });
  try {
    ...
    return res
      .status(500)
      .json({ ok: false, error: error.message || "Could not save video." });
  }
});
...
app.post("/api/stories", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ ok: false, error: error.message || "Could not save story." });
  }
});
...
app.patch("/api/account/profile", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ ok: false, error: error.message || "Could not update profile." });
  }
});
  ...
  try {
    ...
    if (claim.uid) {
      ...
      if (userSnapshot.exists()) {
        ...
        profile = {
          uid: claim.uid,
          userId: value.username || `@${userId}`,
          name: value.name || "Indo User",
        };
      }
    }
    return res.json({
    ...
    return res
      .status(500)
      .json({ ok: false, error: error.message || "Could not check User ID." });
  }
});
  ...
  try {
    ...
    if (existingProfile.exists() && existingProfile.val()?.usernameKey)
      return res
        ...
        .json({
          ok: false,
          error:
            "This account already has a User ID. One user can have only one User ID.",
        });
    ...
    if (!claim.committed)
      return res
        ...
        .json({
          ok: false,
          error: `@${userId} is already taken. Choose another User ID.`,
        });
    const counterRef = db.ref("system/indoCounter");
    const counter = await counterRef.transaction(
      (current) => (Number(current) || 1165) + 1,
    );
    if (!counter.committed) {
      await usernameRef.remove();
      return res
        .status(500)
        .json({ ok: false, error: "Could not generate Indo ID." });
    }
    ...
    return res
      ...
      .json({
        ok: false,
        error: error.message || "Could not create account profile.",
      });
  }
});
...
app.post("/api/account/delete", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ ok: false, error: error.message || "Could not delete account." });
  }
});
...
app.get("/api/social/follow-status/:targetUid", async (req, res) => {
  ...
  try {
    ...
    return res
      ...
      .json({
        ok: false,
        error: error.message || "Could not load follow status.",
      });
  }
});
app.post("/api/account/activity", async (req, res) => {
  ...
  try {
    await db
      .ref(`users/${user.uid}/lastActiveAt`)
...
setInterval(() => {
  cleanupInactiveAccounts({ db, auth }).catch(() => {});
