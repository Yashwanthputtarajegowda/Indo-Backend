...
let telegramRouter;
function getTelegramRouter() {
  if (telegramRouter) return telegramRouter;
  const app = getFirebase();
  const db = app ? getDatabaseWithUrl(DATABASE_URL, app) : null;
  const auth = app ? admin.auth(app) : null;
  telegramRouter = createTelegramChunkRouter({
    express,
    db,
    auth,
    saveVideo: async ({ db: telegramDb, user, upload, streamUrl }) => {
      ...
      const video = {
        ...
        telegram: {
          ...
          chunkSize: Number(upload.chunkSize || 0),
          totalChunks: Number(upload.totalChunks || 0),
  ...
  express.application.use = function indoB2Use(...args) {
    originalUse.call(this, async (req, res, next) => {
      if (req.path.startsWith("/api/telegram/") || req.path.startsWith("/api/media/videos/telegram/")) {
        applyCors(req, res);
        return getTelegramRouter()(req, res, next);
      }
