...
function buildBotPool() {
  ...
  for (let index = 1; index <= MAX_BOTS; index += 1) {
    ...
    if (token && chatId) {
      bots.push({ key: `bot-${index}`, index, token, chatId });
    }
  }
  return bots;
}
...
async function reserveBot(db, bots) {
  ...
  const snapshots = await Promise.all(
    bots.map(async (bot) => {
      const ref = db.ref(`telegramBotState/${bot.key}`);
  ...
  await db.ref(`telegramBotState/${chosen.key}`).transaction((current) => {
    ...
    const stale = Number(state.updatedAt || 0) + LEASE_STALE_MS < Date.now();
    return {
      active: (stale ? 0 : Math.max(0, Number(state.active || 0))) + 1,
      updatedAt: Date.now(),
    };
  });
  return chosen;
...
async function releaseBot(db, bot) {
  ...
  await db.ref(`telegramBotState/${bot.key}`).transaction((current) => {
    const state = current && typeof current === "object" ? current : {};
    return {
      active: Math.max(0, Number(state.active || 0) - 1),
      updatedAt: Date.now(),
    };
  });
}

async function getBotByKey(bots, key) {
...
export function createTelegramChunkRouter({ express, db, auth }) {
  ...
  router.get("/api/telegram/storage", requireUser, (_req, res) => {
  ...
  router.post("/api/telegram/uploads", requireUser, async (req, res) => {
    ...
    if (!totalChunks || !Number.isFinite(size) || size <= 0 || size > 20 * 1024 * 1024 * 1024) {
  ...
  router.post(
    ...
    requireUser,
    express.raw({ type: ["application/octet-stream", "video/*", "application/x-www-form-urlencoded"], limit: "2mb" }),
    async (req, res) => {
      if (!db) return res.status(503).json({ ok: false, error: "Service unavailable." });
      const uploadId = safeUploadId(req.params.uploadId);
      const chunkIndex = safeChunkIndex(req.params.chunkIndex);
      if (!uploadId || chunkIndex === null || !Buffer.isBuffer(req.body) || req.body.length === 0 || req.body.length > CHUNK_SIZE) {
      ...
      try {
        ...
        form.set(
          "document",
          new Blob([req.body], { type: String(upload.mimeType || "application/octet-stream") }),
          `${safeFileName(upload.fileName)}.part${String(chunkIndex).padStart(6, "0")}`,
        );
        ...
        await uploadRef.child("uploadedChunks").transaction((current) => {
          const value = Number(current || 0);
          return value + 1;
        });
        return res.status(201).json({ ok: true, uploadId, chunk });
      } catch (error) {
        if (Number(error?.status) === 429) {
          return res.status(503).json({ ok: false, error: "Telegram is busy. Please retry this chunk.", retryAfter: Number(error?.telegramResponse?.parameters?.retry_after || 5) });
        }
  ...
  router.get("/api/telegram/uploads/:uploadId/chunks/:chunkIndex", requireUser, async (req, res) => {
    ...
    const upload = uploadSnapshot.val() || {};
    const bots = buildBotPool();
    const bot = await getBotByKey(bots, chunk.botKey);
