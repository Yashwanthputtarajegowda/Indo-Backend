...
export const canonicalUserRoot = (uid) =>
  `users/${String(uid || "").trim()}`;
function legacyUserValue(user, field) {
  return user?.profile?.[field] ?? user?.[field];
}
function mergeRelationValues(
  canonicalValue,
  legacyValue,
) {
  const canonical =
...
async function readRelation(db, uid, relation) {
  const [canonicalSnapshot, legacySnapshot] =
    await Promise.all([
      db
        .ref(
          `${canonicalUserRoot(uid)}/social/${relation}`,
        )
        .get(),
      db
        .ref(
          `${canonicalUserRoot(uid)}/${relation}`,
        )
        .get(),
    ]);
  return mergeRelationValues(
...
export async function syncCanonicalUser({
  ...
  if (includeContent) {
    ...
    const videos = videosSnapshot.val() || {},
      ...
      saves = savesSnapshot.val() || {};
    for (const [id, item] of Object.entries(
      videos,
    )) {
      if (
        !item ||
        String(item.ownerUid || "") !== cleanUid
      )
        continue;
      const video = {
        ...item,
      ...
      engagementVideos[id] = {
        ...
        likesCount: Object.keys(likeMap).filter(
          (uidKey) => Boolean(likeMap[uidKey]),
        ).length,
        commentsCount: Object.keys(
          commentMap,
        ).filter((commentKey) =>
          Boolean(commentMap[commentKey]),
        ).length,
        savesCount: Object.keys(saveMap).filter(
          (uidKey) => Boolean(saveMap[uidKey]),
        ).length,
      };
    }
    for (const [id, item] of Object.entries(
  ...
  const profilePrivate = compact({
    email:
      user.profilePrivate?.email ?? user.email,
    phoneNumber:
      user.profilePrivate?.phoneNumber ??
      user.phoneNumber,
    mobile:
      user.profilePrivate?.mobile ?? user.mobile,
  });
  const followersCount =
      Object.keys(followers).length,
    followingCount =
      Object.keys(following).length,
    postsCount = Object.keys(
      contentVideos,
    ).length,
    storiesCount = Object.keys(
      contentStories,
    ).length;
  let viewsCount = 0,
    likesCount = 0,
    commentsCount = 0,
    savesCount = 0;
  for (const engagement of Object.values(
    engagementVideos,
  )) {
...
export function canonicalFollowUpdate({
  ...
  const update = {};
  update[
    `${canonicalUserRoot(followerUid)}/social/following/${targetUid}`
  ] = follow ? targetEntry : null;
  update[
    `${canonicalUserRoot(targetUid)}/social/followers/${followerUid}`
  ] = follow ? followerEntry : null;
  return update;
}
export async function migrateAllUsersToCanonical({
  db,
}) {
  if (!db)
    throw new Error(
  ...
  const byOwnerVideos = {},
    byOwnerStories = {},
    byOwnerEngagement = {};
  for (const [id, video] of Object.entries(
    videos,
  )) {
    ...
    const likeMap = likes[id] || {},
      commentMap = comments[id] || {},
      saveMap = saves[id] || {};
    byOwnerVideos[ownerUid][id] = {
      ...video,
  ...
  for (const [id, story] of Object.entries(
    stories,
  )) {
    const ownerUid = String(
      story?.ownerUid || "",
    ...
    byOwnerStories[ownerUid][id] = {
      ...story,
      id: String(story.id || id),
    };
  }
  for (const uid of Object.keys(users)) {
    ...
    const profilePrivate = compact({
      ...
      phoneNumber:
        ...
        user.phoneNumber,
      mobile:
        user.profilePrivate?.mobile ??
        user.mobile,
    });
    const userVideos = byOwnerVideos[uid] || {},
      userStories = byOwnerStories[uid] || {},
      userEngagement =
        byOwnerEngagement[uid] || {};
    let viewsCount = 0,
      likesCount = 0,
      commentsCount = 0,
      savesCount = 0;
    ...
    updates[`${root}/profilePrivate`] =
      profilePrivate;
    updates[`${root}/verification`] =
      user.verification || {};
    updates[`${root}/settings`] = {
      ...(user.settings || {}),
      accountType:
        user.settings?.accountType ||
        user.accountType ||
    ...
    updates[`${root}/content`] = {
      posts: userVideos,
      videos: userVideos,
      stories: userStories,
    };
    updates[`${root}/engagement`] = {
      videos: userEngagement,
    };
    updates[`${root}/stats`] = {
      postsCount: Object.keys(userVideos).length,
      videosCount: Object.keys(userVideos).length,
      storiesCount:
        Object.keys(userStories).length,
      followersCount:
        Object.keys(followers).length,
      followingCount:
        Object.keys(following).length,
  ...
  await db
    .ref("system/canonicalSchemaVersion")
    .set({
      version: 4,
      migratedAt: Date.now(),
