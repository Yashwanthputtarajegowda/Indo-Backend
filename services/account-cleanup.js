...
export async function cleanupInactiveAccounts({ db, auth, now = Date.now() }) {
  if (!db || !auth) return { checked: 0, deleted: 0, storiesChecked: 0, storiesDeleted: 0, skipped: true };
  const storyCleanup = await cleanupExpiredStories({ db, now });
  const snapshot = await db.ref("users").get();
  if (!snapshot.exists()) return { checked: 0, deleted: 0, ...storyCleanup, skipped: false };
