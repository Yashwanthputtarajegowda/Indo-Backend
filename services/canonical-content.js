...
export async function saveCanonicalStory({ db, uid, story }) {
  ...
  await db.ref(`${canonicalStoryPath(uid, story.id)}`).set(story);
...
export async function saveCanonicalComment({ db, ownerUid, mediaId, comment }) {
  ...
  await db.ref(`${canonicalUserRoot(ownerUid)}/engagement/videos/${mediaId}/comments/${comment.id}`).set(comment);
