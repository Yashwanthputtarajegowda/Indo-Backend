export async function toggleMediaLike({
  ...
  if (!uid || !mediaId)
    throw new Error(
      "User and media are required.",
    );
  const likeRef = db.ref(
    `mediaLikes/${mediaId}/${uid}`,
  );
  ...
  return {
    ...
    likes: Number(likesSnapshot.val()) || 0,
  };
}

export async function getMediaLikeStatus({
  db,
  uid,
  mediaId,
}) {
  ...
  const [likeSnapshot, countSnapshot] =
    await Promise.all([
      db
        .ref(`mediaLikes/${mediaId}/${uid}`)
        .get(),
